...
## Core Workflow
Follow the stages in [references/workflow.md](references/workflow.md):
...
4. **Final Output**: Generate the document using [references/PRD Template.md](references/PRD Template.md).
...
- **Output & Integration (CRITICAL)**: 
  1. **Filename Confirmation**: Before saving any files, suggest a professional name and ask the user to confirm or provide their own.
  2. **Local Markdown**: After name confirmation, save the final PRD as a `.md` file using the `write_file` tool.
  3. **Word Document**: If a `docx` or similar skill is available, generate a corresponding `.docx` file in the workspace using the confirmed name.
- **Template Strictness (CRITICAL)**: 

## Resources
- [references/PRD Template.md](references/PRD Template.md): The official standard template for final output.
- [references/workflow.md](references/workflow.md): Step-by-step guidance.
